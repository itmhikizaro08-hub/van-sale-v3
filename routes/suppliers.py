...
def pay(supplier_id):
    ...
    if request.method == 'POST':
        ...
        amount = round(min(amount, s.outstanding_balance), 2) if s.outstanding_balance > 0 else round(amount, 2)
...
def approve_payment(payment_id):
    ...
    payment.approved_by_id = current_user.id
    payment.approved_at = datetime.utcnow()
    payment.supplier.outstanding_balance = round(max(0, payment.supplier.outstanding_balance - payment.amount), 2)
    db.session.commit()
